<template>
  <div id="session">
    <div class="deck-info">
      <h2>{{statusIcon}} {{this.deckName}}</h2> 
      <h3>{{this.deckDesc}}</h3> 
    </div>
...
export default {
    ...
    saveScore() {
      if(this.userScore > 0) {
        DeckService.updateUserDeckHistory(
          this.deckId, this.currentScore / this.cards.length, this.currentScore);
      } else {
  ...
  created() {
    ...
    DeckService
      ...
      .then(response => {
        ...
        this.deckStatus = response.data.deckStatus;
      });
    CardService
  ...
  computed: {
    ...
    statusIcon() {
      if (this.deckStatus === 1) return '🔒'; // Private
      if (this.deckStatus === 2) return '🥷'; // Unlisted
      if (this.deckStatus === 3) return '👥'; // Public
      return '';
    }
  }
}
