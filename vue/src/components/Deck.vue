<template>
   <div class="deck-container">
    ...
    <div class="buttons">
      ...
      <button class="delete" v-on:click="deleteDeck" 
      v-if="canDelete">
...
export default {
  ...
  computed: {
    ...
    canDelete() {
      ...
      if (this.deck.ownerId == this.$store.state.user.id) {
        return true;
      }
  ...
  methods: {
    ...
    deleteDeck() {
      if (confirm(`Are you sure you want to delete "${this.deck.deckName}"?`)) {
        DeckService
          .deleteDeck(this.deck.deckId)
          .then(() => {
            this.$emit('deck-deleted', this.deck.deckId);
            this.$router.go();
          })
          .catch(() => {
